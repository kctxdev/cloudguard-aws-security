...
- Python 3.12+ (para a Lambda localmente, opcional)
...
> **Atenção:** o ambiente prod requer aprovação manual no GitHub antes do apply (configurado no workflow).
...
> O pipeline usa **OIDC** para autenticar na AWS — sem access keys hardcoded. Veja como configurar: [GitHub OIDC + AWS](https://docs.github.com/en/actions/deployment/security-hardening-your-deployments/configuring-openid-connect-in-amazon-web-services)
...
> O bucket S3 de auditoria tem `force_destroy = true` apenas no DEV. No PROD ele **não será destruído** automaticamente para preservar logs.
